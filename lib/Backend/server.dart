...
/// Wraps everything in runZonedGuarded so unhandled async errors don't crash the server.
  ...
  runZonedGuarded(() async {
    ...
    await for (HttpRequest request in server) {
      ...
      if (WebSocketTransformer.isUpgradeRequest(request)) {
        ...
        print('New WebSocket client: ${ws.hashCode}, handshake took $handshakeMs ms');

        // Store a null SqlConnection for this client until they send "open"
        _connections[ws] = null;

        // Listen for messages from this WebSocket client
        // and handle them in _handleRawData.
        ws.listen(
    ...
    // This catches *any* otherwise unhandled exception or error in the zone
    print('Uncaught error in server zone: $error\n$stackTrace');
  });
}
...
/// Wraps in try/catch to avoid unhandled exceptions from the DB socket close.
...
void _onClientDone(WebSocket ws) {
  ...
  _closeConnectionFor(ws); // We can ignore if it throws
